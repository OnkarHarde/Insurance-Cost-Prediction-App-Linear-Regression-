...
# ============================================================
# 📂 Load Dataset Manually (User Upload)
# ============================================================
st.subheader("📂 Upload Training Dataset")

uploaded_train = st.file_uploader("Upload the training dataset (insurance.csv)", type=["csv"])
...
if uploaded_train is not None:
    ...
    # ============================================================
    df_clean.loc[:, 'sex'] = df_clean['sex'].map({'female': 0, 'male': 1})
    df_clean.loc[:, 'smoker'] = df_clean['smoker'].map({'no': 0, 'yes': 1})
    ...
    # ------------- CSV Upload -------------------
        ...
        if uploaded_file is not None:
            ...
            test_df.loc[:, 'smoker'] = test_df['smoker'].map({'no': 0, 'yes': 1})
    ...
    st.warning("⚠️ Please upload a training dataset to start.")
